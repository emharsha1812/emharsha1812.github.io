...
The `Value` class wraps a number (`data`) and tracks its gradient (`grad`) and the operation that created it (`_op`).
...
2. The `_backward` function defined in `__add__` runs.
3. It creates gradients for `a` and `b` (`a.grad += 1.0 * c.grad`).
...
We use **Adam** optimization. In a loop, we grab a document, run the model forward to get a loss (error), backpropagate to find gradients, and update the weights.
